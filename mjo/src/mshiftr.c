/*
 ...
 and related or neighboring rights to this file (mshiftr.c).
...
extern int
  mjo_r7_shift_right(void *io_vector,
                     size_t const i_slot,
  ...
  do
    {
      ...
      l_slot_src = ((ptrdiff_t)i_slots - 1);
      ...
      do
        {
          if ((ptrdiff_t)i_slot >= l_slot_src)
            {
              break;
            }

          if ((ptrdiff_t)i_slots > l_slot_dst)
            {
              if (i_discharge)
                {
